namespace Chess.Core
{
	...
	{
		public struct SearchToggles
		{
			public bool EnableMateDistancePruning;
			public bool EnableTranspositionTable;
			public bool EnableMoveOrdering;
			public bool EnableExtensions;
			public bool EnableLateMoveReductions;
			public bool EnableKillerHistoryUpdates;

			public static SearchToggles Default => new()
			{
				EnableMateDistancePruning = true,
				EnableTranspositionTable = true,
				EnableMoveOrdering = true,
				EnableExtensions = true,
				EnableLateMoveReductions = true,
				EnableKillerHistoryUpdates = true,
			};
		}
		...
		public SearchToggles Toggles = SearchToggles.Default;
		...
		public NodeStats SearchToDepth(int maxDepth)
		{
			bestEvalThisIteration = bestEval = 0;
			bestMoveThisIteration = bestMove = Move.NullMove;
			...
			CurrentDepth = 0;
			debugInfo = "Starting fixed-depth search with FEN " + FenUtility.CurrentFen(board);
			searchCancelled = false;
			searchDiagnostics = new SearchDiagnostics();
			nodeStats = new NodeStats();
			searchIterationTimer = new System.Diagnostics.Stopwatch();
			searchTotalTimer = System.Diagnostics.Stopwatch.StartNew();
			transpositionTable.enabled = Toggles.EnableTranspositionTable;
		...
		int Search(int plyRemaining, int plyFromRoot, int alpha, int beta, int numExtensions = 0, Move prevMove = default, bool prevWasCapture = false)
		{
			...
			if (plyFromRoot > 0)
			{
				...
				if (Toggles.EnableMateDistancePruning)
				{
			...
			for (int i = 0; i < moves.Length; i++)
			{
				...
				int extension = 0;
				if (Toggles.EnableExtensions && numExtensions < maxExtentions)
				{
				...
				if (Toggles.EnableLateMoveReductions && extension == 0 && plyRemaining >= 3 && i >= 3 && !isCapture)
				{
				...
				if (eval >= beta)
				{
					...
					if (Toggles.EnableKillerHistoryUpdates && !isCapture)
					{
