...
public class EngineUCI
{
	...
	static readonly string[] goLabels = new[] { "go", "depth", "movetime", "wtime", "btime", "winc", "binc", "movestogo" };
	...
	void ProcessGoCommand(string message)
	{
		...
		{
			int depth = TryGetLabelledValueInt(message, "depth", goLabels, 1);
			player.ThinkToDepth(depth);
		}
